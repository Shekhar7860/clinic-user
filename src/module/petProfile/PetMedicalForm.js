...
import UploadingDocs from '../../components/UploadingDocs';

const windowWidth = Dimensions.get('window').width;

const PetMedicalForm = props => {
    ...
    const saveForm = () => {
        if (title === '') {
            ...
            Network('user/save-pet-report', 'post', data, token)
                .then(async res => {
                    console.log(' /n/n Result after submit>>> ', console.log(res.data));
                    showToastMessage(res.message);
                    setLoading(false);
                    navigation.goBack({ tabIndex: tabIndex });
                })
                .catch(error => {
                    console.log(error, 'err is >>.')
                    setLoading(false);
                });
        }
    };
                        ...
                        <View
                            ...
                            <TouchableOpacity
                                ...
                                <View
                                    ...
                                    <Image
                                        source={document !== '' ? { uri: document } : CAMERA}
                                        style={
                                            document !== ''
                                                ? {
                                                    width: '100%',
