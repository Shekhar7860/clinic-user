...
import { CAMERA, GALLERY, MEDICAL_RECORDS } from '../../images';
import {
  BG_COLOR,
  BLACK,
  GRAY_100,
...
const BookingStatus = props => {
  ...
  const referrer = props.route.params?.referrer || 1;
  ...
  const openMap = () => {
    if (
      typeof bookingData?.vendor_latitude != 'undefined' &&
      bookingData?.vendor_longitude != null &&
      bookingData?.vendor_latitude != '' &&
      typeof bookingData?.vendor_longitude != 'undefined' &&
      bookingData?.vendor_longitude != null &&
      bookingData?.vendor_longitude != ''
    ) {
      var lat = `${bookingData?.vendor_latitude}`; // '28.6077438';
      var long = `${bookingData?.vendor_longitude}`; // ',77.3674968';
      var zoom = ',17z';
